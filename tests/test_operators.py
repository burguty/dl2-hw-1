...
def test_sigmoid(a: float, b: float) -> None:
    ...
    * It is  strictly increasing.
    ...
    assert_close(sigmoid(a) + sigmoid(neg(a)), 1.0)

    assert sigmoid(b) >= 0.0
    assert sigmoid(b) <= 1.0
    assert_close(1.0 - sigmoid(b), sigmoid(neg(b)))
    assert_close(sigmoid(b) + sigmoid(neg(b)), 1.0)
...
def test_distribute(a: float, b: float, c: float) -> None:
    ...
    assert_close(mul(b, add(a, c)), add(mul(b, a), mul(b, c)))
...
def test_other(a: float, b: float, c: float) -> None:
    ...
    assert_close(abs(a), abs(neg(a)))
    assert abs(a - b) + abs(b - c) >= abs(a - c)

    if lt(a, b):
        assert_close(max(a, b), b)

    else:
        assert_close(max(a, b), a)
